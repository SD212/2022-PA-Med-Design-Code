<template>
    <div class="problem">
        <button v-on:click="createProblem" class="problem-button">Generate Power Rule Integral Problem</button> <!--creates the problem button which calls createProblem when clicked on-->
        <br>
...
export default {
    ...
    data() {
            return {
                ...
                displayHint1: false,
                hintContent1: "$$∫x^ndx=\\frac{x^{n+1}}{n+1}+C$$",
    ...
    methods: {
        ...
        createProblem() {
            ...
            const newCoefs = [];
            for (let i = 0; i < amtOfTerms; i++) {
                newCoefs[i] = coefs[i].divide(exps[i].add(new Fraction(1, 1)));
            }
            //calculates new exponents
            const newExps = [];
            for (let i = 0; i < amtOfTerms; i++) {
                newExps[i] = exps[i].add(new Fraction(1, 1));
            }
            //generates answer
            ...
            this.formula = "$$∫(" + problem + ")dx$$";
            this.solution = solution + "+c";
            this.displaySolution = displaySolution + "+C";
        ...
        submitMethod() {
            if (this.uinput.toLowerCase() == this.solution) {
                this.correct = true;
                this.incorrect = false;
